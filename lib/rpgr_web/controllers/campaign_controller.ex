defmodule RpgrWeb.CampaignController do
  ...
  def create(conn, %{"campaign" => campaign_params}) do
    with {:ok, %Campaign{} = campaign} <- CampaignContext.create_campaign(campaign_params) do
      conn
      |> put_status(:created)
  ...
  def update(conn, %{"id" => id, "campaign" => campaign_params}) do
    ...
    with {:ok, %Campaign{} = campaign} <- CampaignContext.update_campaign(campaign, campaign_params) do
      render(conn, "show.json", campaign: campaign)
    end
  end
